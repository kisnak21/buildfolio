...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!ingress.success) {
      return jsonResponse(
        { success: false, message: 'Too many AI requests. Try again later.' },
        {
          status: 429,
          headers: {
            'Retry-After': String(Math.ceil(ingress.resetInMs / 1_000)),
          },
        },
        requestId,
      )
    ...
    const onTelemetry = (event: AiTelemetryEvent) => {
      ...
      const metadata = { requestId, task: request.task, ...event }
    ...
    const quotaChecks = await Promise.all([
      rateLimitStatus(hourlyKey, AI_QUOTAS.hourly.config),
      rateLimitStatus(dailyKey, AI_QUOTAS.daily.config),
      rateLimitStatus(globalMinuteKey, GLOBAL_MINUTE_QUOTA),
      rateLimitStatus(globalDayKey, GLOBAL_DAY_QUOTA),
    ])
    const [hourly, daily, globalMinute, globalDay] = quotaChecks

    const quotaFailure = [
      {
        result: hourly,
        message: 'AI hourly limit reached. Try again later.',
      },
      {
        result: daily,
      ...
      {
        result: globalMinute,
        message: 'AI capacity is busy. Try again shortly.',
      },
      {
        result: globalDay,
        message: 'Daily AI capacity has been reached.',
      },
    ...
    if (quotaFailure) {
      ...
      return jsonResponse(
        { success: false, message: quotaFailure.message },
        {
          status: 429,
          headers: {
            'Retry-After': String(
              Math.ceil(quotaFailure.result.resetInMs / 1_000),
            ),
          },
        },
        requestId,
        ...
        async start(controller) {
          ...
          const deadline = setTimeout(
            () =>
              generationController.abort(
                new DOMException(
                  'AI generation deadline exceeded',
                  'TimeoutError',
                ),
              ),
              ...
              logger.warn(
                {
                  ...
                  outputCharacters,
                  outcome: 'failed',
                  errorClass: errorClass(streamError),
                  errorMessage:
    ...
    logger.warn(
      {
        ...
        errorClass: errorClass(routeError),
        errorMessage: httpError(routeError).message || 'AI generation failed',
      },
      status === 499 ? 'AI generation cancelled' : 'AI generation failed',
    )
    return jsonResponse(
      {
        success: false,
